...
class DummySolution(Solution):
    ...
    def init_random(self, problem):
        self.representation = np.array([0.25, 0.5, 0.75, 1.0], dtype=float)
    ...
    def representation_distance_directly(self, other):
        if self.representation is None or other.representation is None:
            return 0.0
        return float(np.linalg.norm(self.representation - other.representation))

    def evaluate(self, problem):
        s = float(np.sum(self.representation))
        sq = float(np.sum(np.square(self.representation)))
        self.objective_values = [s, sq]
        self.fitness_values = [s, sq]
        self.objective_value = s
        self.fitness_value = s
...
class TestMoeadVariationSupportReal(unittest.TestCase):

    def test_copy(self):
        support = MoeadVariationSupportReal(
        ...
        self.assertIsInstance(copied, MoeadVariationSupportReal)
        self.assertEqual(copied.crossover_probability, 1.0)
        self.assertEqual(copied.mutation_probability, 0.1)
        self.assertEqual(copied.eta_c, 20.0)
        self.assertEqual(copied.eta_m, 20.0)
    ...
    def test_generate_offspring(self):
        ...
        parent1 = DummySolution(np.array([0.1, 0.2, 0.3, 0.4], dtype=float))
        parent2 = DummySolution(np.array([0.9, 0.8, 0.7, 0.6], dtype=float))
        child = DummySolution()
        ...
        for value in child.representation:
    ...
    def test_generate_offspring_with_none_representation(self):
        ...
        parent2 = DummySolution(np.array([0.2, 0.3, 0.4, 0.5], dtype=float))
        child = DummySolution()
        optimizer = DummyOptimizer()
